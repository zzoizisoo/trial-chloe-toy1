...
export default () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if(newProfileImg){
                const url = await uploadProfileImgToS3(newProfileImg)
    ...
    const uploadProfileImgToS3 = async (image) => { 
        try {
            const fileId = uuidv4();
            await UploadObject(`userProfileImg/${fileId}.png`, image)            
            return `https://${Meteor.settings.public.S3_BUCKET}.s3.ap-northeast-2.amazonaws.com/userProfileImg/${fileId}.png`
        } catch (err) {
            console.error(err);
        }
    }
